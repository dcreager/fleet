...
add_one(struct flt *flt, void *ud)
{
    ...
    if (i < max) {
        result += i;
        return flt_return_to(flt, add_one, (void *) i+1);
    }
}
...
{
    result = 0;
    flt_fleet_run(fleet, add_one, (void *) min);
}
...
verify(void)
{
    ...
    flt_check_result(sequential_return, "%" PRIu64, result, expected);
    return 0;
}
...
struct flt_example *
sequential_return(uint64_t max_)
{
    result = 0;
    min = 0;
    max = max_;
